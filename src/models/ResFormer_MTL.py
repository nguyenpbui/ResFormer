...
class Res34_Swin_MS(nn.Module):    ### low resolution + Multi-scale
    def __init__(self, image_size, block, layers, seg_num_classes, cls_num_classes,
        ...
        self.fem4 = FEM_4(channels[3], channels[3])
        self.fem3 = FEM_4(channels[2], channels[2])
        self.fem2 = FEM_4(channels[1], channels[1])
        self.fem1 = FEM_4(channels[0], channels[0])

        self.cls_conv_3 = nn.Sequential(nn.BatchNorm2d(512),
                                        nn.ReLU(inplace=True),
                                        nn.AdaptiveAvgPool2d(1))

        self.cls_conv_2 = nn.Sequential(nn.BatchNorm2d(256),
                                        nn.ReLU(inplace=True),
                                        nn.AdaptiveAvgPool2d(1))
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        d4 = self.decode4(self.fem4(encoder[3]), self.fem3(encoder[2]))
        d3 = self.decode3(d4, self.fem2(encoder[1]))
        d2 = self.decode2(d3, self.fem1(encoder[0]))
