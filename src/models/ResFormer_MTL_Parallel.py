...
class Res34_Swin_MS(nn.Module):    ### low resolution + Multi-scale
    def __init__(self, image_size, block, layers, seg_num_classes, cls_num_classes,
        ...
        patches_resolution = [self.image_size//2, self.image_size//2] # //2
        ...
        self.decode0 = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
                                     nn.Conv2d(channels[0], seg_num_classes, kernel_size=1, bias=False))

        self.cls_conv_3 = nn.Sequential(nn.BatchNorm2d(512), nn.ReLU(inplace=True), nn.AdaptiveAvgPool2d(1))
        self.cls_conv_2 = nn.Sequential(nn.BatchNorm2d(256), nn.ReLU(inplace=True), nn.AdaptiveAvgPool2d(1))
        
        self.fc1 = nn.Linear(768, 32)
        self.fc2 = nn.Linear(32, cls_num_classes)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = self.maxpool(x)
        B, L, C = ms1.shape
        ms1 = ms1.view(B, int(np.sqrt(L)), int(np.sqrt(L)), C).permute(0,3,1,2)
        x = x + ms1
        
        x_c = self.layer1(x)
        x = x.flatten(2).transpose(1, 2)
        x_t = self.swin_layers[0](x)
        B, L, C = x_t.shape
        x_t = x_t.view(B, int(np.sqrt(L)), int(np.sqrt(L)), C).permute(0,3,1,2)
        x = x_c + x_t
        encoder.append(x)
        ...
        x_t = self.MS4(x_t)
        x_t = x_t.flatten(2).transpose(1, 2)
        x_t = self.swin_layers[3](x_t)
        B, L, C = x_t.shape
        x_t = x_t.view(B, int(np.sqrt(L)), int(np.sqrt(L)), C).permute(0,3,1,2)
        x = x_c + x_t
        encoder.append(x)
        
        out_3 = self.cls_conv_3(encoder[3])
        out_2 = self.cls_conv_2(encoder[2])
        out_c = torch.cat((out_3, out_2), dim=1)
        out_c = out_c.view(out_c.size(0), -1)
        out_c = self.fc1(out_c)
        out_c = self.fc2(out_c)
        
        d4 = self.decode4(encoder[3], encoder[2]) 
        d3 = self.decode3(d4, encoder[1])
        d2 = self.decode2(d3, encoder[0])
        out_s = self.decode0(d2)

        return out_s, out_c # type: ignore
    
def res34_swin_MS(image_size, seg_num_classes, cls_num_classes) :
    return Res34_Swin_MS(image_size, BasicBlock, [3, 4, 6, 3], seg_num_classes=seg_num_classes, cls_num_classes=cls_num_classes)
